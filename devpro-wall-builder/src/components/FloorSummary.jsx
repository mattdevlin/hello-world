...
import { FLOOR_THICKNESS, FLOOR_EPS_DEPTH, FLOOR_SPLINE_EPS_DEPTH,
...
export default function FloorSummary({ layout, floorName, projectName }) {
  ...
  const unreinforcedVol = unreinforcedSplines
    .reduce((sum, s) => sum + s.width * s.length * FLOOR_SPLINE_EPS_DEPTH, 0);
  ...
  const unreinforcedSA = unreinforcedSplines.reduce((sum, s) => sum + s.width * s.length, 0);
