...
export default function RoofPanelPlans({ layout, roofName, projectName }) {
  ...
  return (
    <div ref={sectionRef} data-print-section style={{ background: '#fff', borderRadius: 8, padding: 16, border: '1px solid #e0e0e0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: '#333' }}>Panel Cut Plans — {roofName}</div>
        <PrintButton sectionRef={sectionRef} label="Panel Plans" projectName={projectName} wallName={roofName} />
      </div>

      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
        {panels.map((panel) => (
          <PanelCard key={panel.globalIndex} panel={panel} />
        ))}
      </div>
    </div>
  );
...
function PanelCard({ panel }) {
  ...
  const hasCuts = (panel.penetrationCuts || []).length > 0;
  const label = `P${panel.globalIndex + 1}`;
  const subtitle = `${Math.round(panel.width)} × ${Math.round(panel.length)} mm`;
  const notes = [];
  if (panel.type === 'cut') notes.push('Cut');
  if (panel.penetrationCuts && panel.penetrationCuts.length) {
    notes.push(`Pen: ${panel.penetrationCuts.join(', ')}`);
  }
