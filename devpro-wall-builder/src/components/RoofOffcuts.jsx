...
export default function RoofOffcuts({ layout, roofName, projectName }) {
  const sectionRef = useRef(null);
  if (!layout || !layout.panels || layout.panels.length === 0) return null;

  const offcuts = computeRoofOffcuts(layout);
  if (offcuts.length === 0) {
    return (
      <div ref={sectionRef} data-print-section style={styles.container}>
        <div style={styles.header}>
          <div style={styles.title}>Offcuts — {roofName}</div>
          <PrintButton sectionRef={sectionRef} label="Offcuts" projectName={projectName} wallName={roofName} />
        </div>
  ...
  return (
    <div ref={sectionRef} data-print-section style={styles.container}>
      <div style={styles.header}>
        <div style={styles.title}>Offcuts — {roofName} ({offcuts.length} pieces)</div>
        <PrintButton sectionRef={sectionRef} label="Offcuts" projectName={projectName} wallName={roofName} />
      </div>
...
function computeRoofOffcuts(layout) {
  const offcuts = [];
